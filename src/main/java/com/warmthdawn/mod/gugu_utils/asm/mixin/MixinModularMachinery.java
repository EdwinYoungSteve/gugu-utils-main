...
import com.warmthdawn.mod.gugu_utils.modularmachenary.ColorableTileCompat;
...
public class MixinModularMachinery {
    public static void inject_tryColorize(TileEntity thisObj, BlockPos pos, int color, TileEntity te) {
        // 使用兼容性适配器处理原版和MMCE的接口差异
        if (ColorableTileCompat.isColorable(te)) {
            ColorableTileCompat.setMachineColor(te, color);
